...
class WheelLeggedEnv:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, curriculum_cfg, 
        ...
        self.rew_survive = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def step(self, actions):
        ...
        # self.terrain_buf &= self.command_ranges[:, 2, 1] > self.command_cfg["ang_vel_range"][1] * 0.8
        #固定一部分去地形
        if(self.mode):
    ...
    def _reward_tracking_lin_x_vel(self):
        ...
        lin_vel_reward = torch.square(self.commands[:, 0] - self.base_lin_vel[:, 0])
        lin_vel_reward = torch.exp(-lin_vel_reward / self.reward_cfg["tracking_linx_sigma"])
    ...
    def _reward_ang_acc_xy(self):
        # Penalize xy axes base angular velocity
        return torch.sum(torch.square(self.base_ang_acc[:, :2]), dim=1)

    def _reward_collision(self):
        # 接触地面惩罚 力越大惩罚越大
        collision = torch.zeros(self.num_envs,device=self.device,dtype=gs.tc_int)
        # collision = torch.ones((self.num_envs,), device=self.device, dtype=gs.tc_int)
        for idx in self.reset_links:
            # collision += torch.square(self.connect_force[:,idx,:]).sum(dim=1)
            collision |= torch.abs(self.connect_force[:,idx,:]).sum(dim=1) > 0
