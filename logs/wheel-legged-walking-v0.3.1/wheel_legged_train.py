...
def get_train_cfg(exp_name, max_iterations):
    ...
    train_cfg_dict = {
        "algorithm": {
            "clip_param": 0.20, #剪裁低可能会带来高noise_std
            "desired_kl": 0.01, #裁减
...
def get_cfgs():
    env_cfg = {
        ...
        "termination_if_roll_greater_than": [25,45],  # degree
        "termination_if_pitch_greater_than": [25,45], #15度以内都摆烂，会导致episode太短难以学习
        ...
        "decimate_aggressiveness": 8,    #优化等级0-8 0：无损 2：原始几何体 5：有明显变化 8： 大变特变
    ...
    reward_cfg = {
        ...
        "feet_distance":[0.25, 0.6], #脚间距范围 m
        "reward_scales": {
            ...
            "lin_vel_z": -0.1, #大了影响高度变换速度 -0.001
            ...
            "ang_vel_xy": -0.1,
            "ang_acc_xy": -1e-6,
            "collision": -100.0,  #base接触地面碰撞力越大越惩罚，数值太大会摆烂 -0.0015
            "feet_distance": -100.0,
            "survive": 1.0,
            "tsk": -2.0, #高速对抗
        },
    ...
    curriculum_cfg = {
        ...
        "curriculum_ang_vel_step":0.0005,   #比例   0.0005
        ...
        "ang_vel_err_range":[0.55,1.0],  #[0.5,1.0]    
    }
...
def main():
    ...
    env = WheelLeggedEnv(
        ...
        show_viewer=False, num_view = 8192,
    )
