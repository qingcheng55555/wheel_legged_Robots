...
def get_cfgs():
    env_cfg = {
        ...
        "termination_if_roll_greater_than": [20,21],  # degree
        "termination_if_pitch_greater_than": [20,21], #15度以内都摆烂，会导致episode太短难以学习
    ...
    command_cfg = {
        ...
        "inverse_tsk": 1.0,    #std = inverse_tsk / angv 这个数值可以用std=1时估计inverse_tsk 越低在高速情况下越贴近0
        "inverse_leg_length": 1.0,   #std = inverse_leg_length / angv 如上同理 越低在高速情况下两腿越相似
    }
    # 课程学习，奖励循序渐进 待优化
    curriculum_cfg = {
        ...
        "err_mode": True, #误差模式 True  比例模式 False
        "lin_vel_err_range":[0.35,0.5],  #课程误差阈值 误差 or 比例 [0.35,0.5]
        "ang_vel_err_range":[0.5,1.0],  #课程误差阈值 误差 or 比例 [0.5,1.0]
    }
