...
class WheelLeggedEnv:
    ...
    def curriculum_commands(self):
        ...
        if self.curriculum_cfg["err_mode"]:
            lin_err_low = self.curriculum_cfg["lin_vel_err_range"][0]
            lin_err_high = self.curriculum_cfg["lin_vel_err_range"][1]
        else:
            mean = self.command_ranges[:, 0, 1].mean()  
            lin_err_low = self.curriculum_cfg["lin_vel_err_range"][0] * mean  
            lin_err_high = self.curriculum_cfg["lin_vel_err_range"][1] * mean
            
        if self.mean_lin_vel_error < lin_err_low:
        ...
        angv_err_high = 0
        if self.curriculum_cfg["err_mode"]:
            angv_err_low = self.curriculum_cfg["ang_vel_err_range"][0]
            angv_err_high = self.curriculum_cfg["ang_vel_err_range"][1]
        else:
            mean = self.command_ranges[:, 2, 1].mean()  
            angv_err_low = self.curriculum_cfg["ang_vel_err_range"][0] * mean  
            angv_err_high = self.curriculum_cfg["ang_vel_err_range"][1] * mean

        if self.mean_ang_vel_error < angv_err_low:
