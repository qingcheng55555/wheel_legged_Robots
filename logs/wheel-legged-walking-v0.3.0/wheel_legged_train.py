...
def get_cfgs():
    env_cfg = {
        ...
        "termination_if_roll_greater_than": [20,45],  # degree
        "termination_if_pitch_greater_than": [20,45], #15度以内都摆烂，会导致episode太短难以学习
    ...
    command_cfg = {
        "num_commands": 6,
        "lin_vel_x_range": [-2.2, 2.2], #修改范围要调整奖励权重 低速范围约[-1.0,1.0]
    ...
    curriculum_cfg = {
        ...
        "curriculum_ang_vel_step":0.00001,   #比例   0.0005
        ...
        "lin_vel_err_range":[0.3,0.45],  #[0.35,0.5]  课程误差阈值(上升/下降) 误差 or 误差比例(上升/下降)上升阈值会在前期从下降阈值误差下降到设定值
