...
def get_cfgs():
    ...
    # 课程学习，奖励循序渐进 待优化
    curriculum_cfg = {
        "curriculum_lin_vel_step":0.0006,   #比例    0.001
        "curriculum_ang_vel_step":0.0001,   #比例   0.0005
        "curriculum_lin_vel_min_range":0.3,   #比例 
        "curriculum_ang_vel_min_range":0.05,   #比例 
        "err_mode": True, #误差模式 True  比例模式 False  要注意在地形情况下速度跟踪肯定和平地差距很大注意误差调整
        "lin_vel_err_range":[0.35,0.6],  #[0.35,0.5]  课程误差阈值(上升/下降) 误差 or 误差比例(上升/下降)上升阈值会在前期从下降阈值误差下降到设定值
        "ang_vel_err_range":[0.5,1.0],  #[0.5,1.0]    比例会按照 实际上升阈值 = (下降阈值-上升阈值)*e^(-(当前范围/设定范围)^2/0.1)+上升阈值 (desmos函数图像R_{2}\left(x\right)=\left(0.5-r\right)e^{-\frac{x^{2}}{0.1}}+r\left\{0<x<1\right\})
    }
